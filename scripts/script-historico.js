  ...
  const carregarHistorico = async () => {
    ...
    historicoNuvem.forEach((item, index) => {
      ...
      card.innerHTML = `
        ...
        ${linkForense(item)}
        <button class="btn-apagar-item" data-id="${escaparHtml(item.id)}" data-url="${escaparHtml(item.imagem_original)}" title="Apagar análise" style="background: none; border: none; color: #ff6b6b; cursor: pointer; padding: 8px; transition: opacity 0.2s;">
